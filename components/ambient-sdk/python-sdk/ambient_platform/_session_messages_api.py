...
class SessionMessage:
    ...
    def _from_proto(cls, pb: object) -> SessionMessage:
        ...
        if ts is not None:
            try:
                created_at = datetime.fromtimestamp(ts.seconds + ts.nanos / 1e9, tz=timezone.utc)
            except Exception:
                pass
        return cls(
            id=getattr(pb, "id", ""),
...
class SessionMessagesAPI:
    ...
    def push(
        ...
        *,
        ...
        try:
            ...
            return SessionMessage._from_proto(pb)
        except grpc.RpcError as exc:
            logger.warning(
                "PushSessionMessage RPC failed (session=%s event=%s): %s",
                session_id, event_type, exc,
            )
            return None
        except Exception as exc:
            logger.warning(
                "PushSessionMessage unexpected error (session=%s): %s",
                session_id, exc,
            )
            return None
    ...
    def watch(
        ...
        *,
        ...
        for pb in stream:
            yield SessionMessage._from_proto(pb)
...
from google.protobuf import descriptor_pool as _pool_module  # noqa: E402
